...
<Availability editions={['portal']} note="Off unless your clinic switches Messaging on — it is off to start with, and your clinic's plan has to include it." />
...
## When the clinic is away

If your clinic is away — closed for a holiday, say, or the person looking after your messages is out — a notice appears above the message box before you start typing. It says until when, for example:

> We're away until Mon, Oct 12 and will reply when we're back. If this is urgent or an emergency, call the clinic or your local emergency number.

Your clinic chooses one of two ways to handle messages while it is away:

- **You can still send.** Your message is delivered as normal and waits for the team. You get one automatic reply with the same wording — once for that time away, not for every message.
- **Messaging is paused.** The notice says "Messaging is paused while the clinic is away", and you cannot send a message or attach a file until the clinic is back. Anything you have typed stays in the box. Earlier messages can still be read.

Either way, do not wait for a reply about anything urgent — call the clinic or your local emergency number.
