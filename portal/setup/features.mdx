...
<Availability editions={['clinic', 'lims']} plans="Any paid plan; Messaging and Notifications need Essential or above" roles="Owner" note="On Diagnostics only the Enterprise plan unlocks Messaging and Notifications — the other lab tiers are not named Essential or Team." />
...
| **Client submitted a form** | _Coming soon_ — not yet active |
| **SMS appointment reminders** | _Coming soon_ — the portal does not send text messages |
| **Email appointment reminders** | One reminder email to each patient before a confirmed visit. When it goes is set on each client record — from 30 minutes to 1 week before; a client nobody has changed is reminded 24 hours before. See [Add your first client](/practice/get-started/add-clients) |
...
Settings marked _Coming soon_ do nothing yet. Do not build a workflow on them, and do not promise patients text-message reminders.
...
| **Require two-factor authentication for clients** | _Coming soon_ — not yet active |
...
<AccordionGroup>
  ...
  <Accordion title="Nobody is receiving appointment reminders">
    Reminders are email only and go to patients who have **confirmed** their appointment. Check **Email appointment reminders** is on, that the patient has selected **Confirm** in the portal, and that **Upcoming appointments** is still switched on for them on their client record. There are no SMS reminders.
  </Accordion>
