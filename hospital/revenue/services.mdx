...
Once Services opens, nothing narrows further:

| Action | Who by default | Governed by |
| --- | --- | --- |
| Read the catalogue | Anyone the role navigation lets in | Role → route |
| Add a service or a category | Anyone who can open Services | Nothing |
| Change a price | Anyone who can open Services | Nothing |
| Put a service on an invoice | Anyone who can open Billing | Nothing |
| See the earnings figures on **Overview** | Anyone who can open Services | Nothing |

Your hospital's staff sheet does carry a **Billing** section of per-person toggles — **Create billing documents**, **Add client payments**, **Add insurance payments**, **Edit service fees**, **View Payment Reports**, **View financial dashboard**, **Refund payments** — and they are recorded against each person. No billing screen reads them, so they change nothing today. Restrict billing at the **Role Navigation** layer instead. That includes **Edit service fees**: repricing is not separated from creating today, so the way to stop a price list being edited by everyone is to keep **Billing** out of most roles' navigation.
