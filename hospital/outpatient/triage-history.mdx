...
**By Patient** groups every triage record into one row per patient — how many, their last triage date, and the most recent status — with a search box and a **date range** filter (leave the range blank to see a patient's whole recorded history). Selecting a patient opens their triage records for that range as vitals cards: blood pressure, pulse, respiratory rate, oxygen saturation, temperature, weight, height and blood sugar, each with its unit shown.

**All Records** opens on the line "View complete history of all triage records". It is a separate screen from the list, not a filter on it, and its toolbar is deliberately shorter: **Search for patient**, **New Patient** and **View Columns**. There is no **New Triage** button and no **Refresh** — this tab is for reading, and new records are taken on the list.
...
- **Vital Signs** — every reading taken, each as a labelled badge: **Blood Pressure** in mmHg, **Pulse Rate** in bpm, **Temperature** in °C, **Respiratory Rate** in bpm, **Oxygen Saturation** as a percentage, **Weight** in kg, **Height** in cm, and **Fasting** and **Random Blood Sugar** in mmol/L. Anything that was not recorded reads **N/A**. A blood sugar recorded before the unit was standardised may show with an explicit **mg/dL** label — it is displayed exactly as it was taken, in the unit it was taken in.
...
{/* source of truth: app/u/clinic/triage/history/, components/clinic/triage-history/ (triage-history-actions.tsx, table-columns.ts, past-triage-form.tsx), components/clinic/general/ (patient-history-group-panel.tsx, patient-history-drilldown-sheet.tsx, patient-history-record-cards.tsx), db/actions/triage-history-grouped.ts, lib/clinic-nav-registry.ts */}
