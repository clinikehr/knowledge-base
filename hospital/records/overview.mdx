...
The heading is **Records**. A **Refresh** button sits beside it.
...
| **Needs chasing** | Anything sent to a patient that wants a reminder |
| **Expiring or expired** | Anything whose link needs re-sending |
...
Everything you have sent a patient and not yet had back appears here, with **Patient**, **Waiting on**, **Sent**, **Last opened**, **Reminders** and **Status**.

**Last opened** is the most recent time the patient opened that item — "today", "3 days ago", or **Not opened** if they never have. It separates a patient who has not seen the request from one who looked and stopped, which is the difference between "did the email arrive?" and "is something on the page stopping them?". It shows a dash when that time could not be looked up just now; a dash never means "not opened". On a narrow screen the column is hidden.

The **Waiting on** column names the item, and tags it when it is not a document: a **form** is waiting on an answer, an **envelope** is waiting on a signature in a signature request. That matters because the remedy is different — a form is chased through the form's own invite list, a signature request through its tracking page, and a document through **Forms → eSignatures**.

Only people who are on a patient's record appear here. A form or a signature request sent to a contact or to a bare email address is tracked on the form or the signature request itself, not on this desk.

This desk uses the same list as the patient's **Documents & forms** tab, so an item never shows one state here and another on the chart. An item whose link has run out leaves this desk straight away, and the patient's record shows it as **Expired** — there is nothing left to chase on a link that no longer opens. Send it again from the patient's record.
...
This tab tells you what each row needs; for a document you act on it on **Forms → eSignatures**, where **Resend** lives — see [Send and track documents](/hospital/records/documents). The footer states your hospital's thresholds — how many days before a document is flagged as not opened, how many before no response, and how close to the deadline counts as expiring. Change them in **Settings → Facility** — see [Facility settings](/hospital/settings/facility).

Empty state: **All clear** — "Nothing is waiting on a response or a signature."

## Old document signing links — owners and managers

Owners and managers see a notice above the queues while anything still depends on the earlier document signing links:

- how many documents sent with those links are still waiting, and the date the last of those links stops working;
- how many appointment defaults still send an old document template, with their names.

**Convert these defaults** opens **Settings → Patient Experience → Sharable Documents**, where each old template has **Convert to form** and then **Use this form here**. **See what is still waiting** opens **Forms → eSignatures**. The notice disappears when both counts reach zero, and each owner and manager gets one notification saying so. Other staff do not see it.

## Unfiled — give every file a folder

Uploads that arrived without a home. Each row shows the **Patient**, the **File**, how long ago it was **Uploaded**, and a **File it** button that opens that patient's record on its **Documents & forms** tab so you can put it where it belongs.
...
<AccordionGroup>
  ...
  <Accordion title="I cannot find a specific document here">
    Records only shows what is outstanding. Use **Send for signature**, which opens **Forms → eSignatures**, for the full list.
  </Accordion>
