...
<Note>
  **Not every case on this list was booked here.** A midwife can put an emergency caesarean straight onto it from a woman's row on [Labour Ward](/hospital/maternity/labour-ward), without leaving the labour board. It arrives as an ordinary case — same numbering, same checklist, same statuses — usually at **Emergency** urgency and often with no theatre assigned yet. Assign the room from this board.
</Note>
...
<Steps>
  ...
  <Step title="Name the care team">
    ...
    If there is nobody to choose, the three pickers are greyed out and a line beneath them says which of two things happened — "Nobody at this clinic has a name on file to pick — the case still books, but with nobody recorded on the team.", or "The staff list could not be loaded — the case still books, but with nobody recorded on the team. Close and reopen this form to try the list again." The case books either way, but the team is named here and only here, so on the second message it is worth reopening the sheet before you book.
  </Step>
  <Step title="Select Book case">
