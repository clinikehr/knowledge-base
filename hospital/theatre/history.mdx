...
<AccordionGroup>
  ...
  <Accordion title="Can I reschedule a case from history?">
    Only if it is still live. Open the case to see its detail panel and choose **Reschedule** — it appears for cases that are scheduled or in theatre, and needs the **Book and reschedule cases** permission. A completed or cancelled case refuses: "This case is completed and can no longer be rescheduled."

    Rescheduling moves the existing case, keeping its number, notes and history. It is not the same as cancelling and re-booking, which ends one case and starts another.
  </Accordion>
</AccordionGroup>
