...
Each test also shows what it bills — the panel's own price from your [Lab station](/hospital/diagnostics/lab-station) catalogue, or, when only specific parameters were requested and every one of those has its own price set, the sum of just those. An item with no price set yet shows **"No price set"** rather than a $0, and the card totals every test at the top. None of this appears for a clinic marked **free** in Settings — no prices, no total, nothing to collect.

**Download requisition**, beside the number of tests, downloads one PDF with **every** test ordered in that visit or ward stay — each test with the parameters that were asked for, the clinical information from the visit, the patient's latest weight, and the requesting clinician's name and licence. It is marked **"Requisition only — no results are recorded on this document"**, so it can never be mistaken for a completed report; the completed report itself downloads from the same row once the test moves to **Completed** (see [Lab reports](/hospital/diagnostics/lab-reports)).

The requisition shows no prices: tests ordered in a visit or on a ward are charged through that patient's bill. It carries the requesting clinician's signature when they have saved one under **Account → My signature**, and otherwise reads "Electronically signed by" and their name.
...
<AccordionGroup>
  ...
  <Accordion title="What does Lab Test History show?">
    Two tabs. **By Patient** (the default) groups every test into one row per patient — how many, their last test date, and the most recent status — with a search box and a date-range filter; leave the range blank to see a patient's whole history, or narrow it to a specific period. Selecting a patient opens their tests for that range as cards (test name, result, status). **All Records** is the original one-row-per-test table, for finding a single request fast.
  </Accordion>
...
{/* source of truth: components/clinic/lab/, app/u/clinic/lab/[clinicId]/page.tsx, app/u/clinic/lab/history/[clinicId]/page.tsx, components/clinic/general/ (patient-history-group-panel.tsx, patient-history-drilldown-sheet.tsx, patient-history-record-cards.tsx), db/actions/lab-history-grouped.ts, lib/clinic-nav-registry.ts */}
