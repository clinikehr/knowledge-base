...
Each test also offers **Download Requisition** — a server-generated PDF of what was ordered (the test, its parameters, the clinical information from the visit, and the requesting clinician's name and licence). It is explicitly marked **"Requisition only — no results are recorded on this document"**, so it can never be mistaken for a completed report; the completed report itself downloads from the same row once the test moves to **Completed** (see [Lab reports](/hospital/diagnostics/lab-reports)).
