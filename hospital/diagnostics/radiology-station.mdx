...
<Availability editions={['clinic']} plans="Enterprise" roles="Any staff member" note="Needs the Diagnostics care area · with Role Navigation on, no built-in role rule reaches the Radiology Station" />
...
The station itself checks nothing: anyone who reaches it can use every tab. Where role matters is [Role navigation](/hospital/settings/role-navigation) — and if it is switched on, none of the built-in role rules include the Radiology Station, so a radiologist keeps **Radiology** but is turned away from the station until an owner adds it to their rule.

| Action | Who by default | How it is governed |
| --- | --- | --- |
| Read the Analytics tab | Anyone who can open the station | Nothing |
| Add, edit or delete a catalogue study | Anyone who can open the station | Nothing |
| Open the **Modalities** tab | Nobody on a Clinic & Hospital plan today | Plan — the imaging bridge ships with the Diagnostics edition from **Professional Lab** upward |
| See prices and revenue figures | Anyone who can open the station | Nothing — a hospital workspace has no permission that hides money |
...
<AccordionGroup>
  ...
  <Accordion title="The Modalities tab is missing">
    The imaging bridge is not part of any Clinic & Hospital plan — it ships with the Diagnostics edition from **Professional Lab** upward — so on a hospital the tab is simply absent. Everything else on this page works without it. See [Modalities](/hospital/diagnostics/modalities).
  </Accordion>
