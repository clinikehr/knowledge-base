...
<Availability editions={['clinic']} plans="Enterprise" roles="Owners, managers, radiologists, radiology technicians, doctors and nurses" note="Needs the Diagnostics care area · with Role Navigation on, no built-in role rule reaches the Radiology Station" />
...
| Submit a report at all | Anyone with the above | Two-factor, but only in a hospital that requires it — where it does, an unverified session is refused with **Two-factor verification is required.** |
| See payment and revenue | Anyone who can open the page | Nothing — a hospital workspace has no permission that hides money |
...
<AccordionGroup>
  <Accordion title="Two-factor verification is required.">
    Your hospital requires two-factor for staff, so writing to a patient record needs a verified session. You are taken to the verification step automatically — complete it and submit again. A hospital that does not require two-factor never sees this. See [Two-factor authentication](/platform/security/mfa).
  </Accordion>
