...
Imaging works like the lab: a clinician requests the study, the modality performs it, a radiologist reports it, and the signed report goes back to the requester. This page walks that loop inside a hospital workspace.

**Radiology** and **Laboratory** both come from one switch — the **Diagnostics** care area in **Settings → Facility**. See [Care areas](/hospital/get-started/care-areas).

<Path steps={['Sidebar', 'Radiology', 'Radiology Station']} />

<TaskHeader before="The Diagnostics care area switched on, and a patient with an imaging request" time="Minutes to request; reporting depends on the radiologist" after="A signed radiology report attached to the patient's record" />
...
<Steps>
  <Step title="Order from the consultation">
    Open the [consultation](/hospital/outpatient/consultation), go to the **Orders** tab and select the **Imaging** sub-tab. Ordering here keeps the request tied to the encounter and the clinical question behind it.
  </Step>
...
<Steps>
  <Step title="Open the study">
    From the imaging list, select **Create Report** on the row. If images are attached the report screen says "{'\{n\}'} images attached" and offers **Open full viewer**, **Download study** and **Archive to cloud**.
  </Step>
...
<AccordionGroup>
  <Accordion title="There is no Radiology group in my sidebar">
    Either the **Diagnostics** [care area](/hospital/get-started/care-areas) is off, or your workspace is not on the Enterprise plan. See [Change your plan](/platform/plans/change-plan).
  </Accordion>
...
<Columns cols={2}>
  ...
  <Card title="Order and report lab tests" icon="/images/icons/flask.svg" href="/hospital/diagnostics/laboratory">
