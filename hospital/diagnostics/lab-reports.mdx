...
**Overview** carries four tiles: **Total Tests**, **Pending Tests**, **Completed** and **Critical Results**, with **Test Volume Trends** and **Status Distribution** beneath them and a **Recent Lab Reports** list at the bottom. A quiet period reads **No lab reports found for this period**.

The **Lab Reports** tab is the working list — **Complete list of all laboratory tests** — with **Search reports...** and a status filter offering **All Status**, **Pending**, **In Progress**, **Completed** and **Cancelled**.
...
<Steps>
  ...
  <Step title="Check the money">
    **Order Summary** → **Order Items** shows subtotal, tax, any flat-rate credit and the **Total Amount**. Tick **Collect Payment Now** and pick a **Payment Method** to take it at the desk; leave it, and the order carries **Payment Pending** with the balance shown.
  </Step>
  <Step title="Create the order">
    **Create Lab Orders (\{n\})**, or **Create &amp; Pay (\{n\})** when collecting. The confirmation offers **Print Receipt**.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Save the lab test before downloading its report.">
    You are trying to download a report for an order that has not been saved yet. Save it, reopen it, then use **Download report**.
  </Accordion>

  <Accordion title="Lab tests ordered. Payment failed">
    The order exists; only the payment did not go through. It carries **Payment Pending** with the outstanding balance — take it again from [Billing](/hospital/revenue/billing) rather than re-ordering the test.
  </Accordion>
