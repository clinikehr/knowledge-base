...
<Availability editions={['clinic']} plans="Enterprise" roles="Owners, managers, lab scientists, doctors and nurses" note="Needs the Diagnostics care area · with Role Navigation on, no built-in role rule reaches the Lab Station" />
...
A downloaded report for a walk-in test — one not ordered from a visit or a ward — ends with a **Payment** section: the amount, what has been paid, the balance and the payment status, in your clinic's currency. A test ordered in a visit or on a ward shows no price; it is charged through that patient's bill. Once the test is **Completed**, the report carries the scientist's saved signature (**Account → My signature**), or "Electronically signed by" and their name.
...
| Take payment on the order | Anyone who can open the report | Nothing |
| See revenue on the Analytics tab | Anyone who can open the page | Nothing — a hospital workspace has no permission that hides money |
