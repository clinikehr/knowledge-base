...
<TaskHeader
  before="Wards, sub-wards and beds created in Settings, and at least one clinician on the clinic's staff list"
  time="About a minute to admit"
  after="A patient occupying a bed, with an attending clinician named and a live chart against their stay"
/>
...
<Steps>
  <Step title="Select Admit Patient on the consultation">
    Open the patient's [consultation](/hospital/outpatient/consultation) and select **Admit Patient**. A bed picker opens and walks the same ward → sub-ward → bed order as this page. A sub-ward with nothing free is marked **Full**, and picking it says **No free beds in this sub-ward** rather than offering nothing — free a bed or choose another sub-ward. Pick a free bed.
  </Step>
  <Step title="Choose the Attending clinician">
...
**SnapShot · Flowsheets · MAR · Orders · Notes · Results · Care History · Problem List · Care Plan · Medications · Maternity · History · Media · Mail**

**Maternity** appears only for an admission with a labour or an open pregnancy behind it — see below.

**Mail** lists the email conversations somebody has linked to this patient, and follows them between admissions the way the **Problem List** does — correspondence belongs to the person, not the stay. It is read-only here: a conversation is put on a chart from [Mail](/hospital/overview/mail) itself.

The tabs that list a long record — **History**, **Media** and the **Earlier results** section of **Results** — show 10 entries at a time, with a **per page** control and a count under them.
...
The chart's **Admission** card, in the left-hand rail, carries the stay's basics — **Ward**, **Bed**, **Admitted**, **Day of stay** and **Attending**. Select the pencil next to the card's title to open **Treatment team**.
...
**Treatment team** opens read-only — nothing can be added or removed — when you are offline or the patient has already been discharged; the current team and its history are still visible.
...
<AccordionGroup>
  ...
  <Accordion title="No clinicians are recorded on this clinic's staff list yet, so this patient cannot be admitted">
...
<AccordionGroup>
  ...
  <Accordion title="Can I change who is attending partway through a stay?">
...
{/* source of truth: components/clinic/ward-admissions/, components/clinic/ward-admissions/chart/, components/clinic/ward-admissions/chart/care-team-sheet.tsx, components/clinic/clients-management/client-mail-tab.tsx, lib/ward/care-team.ts, lib/ward/header-labels.ts, lib/edition-staff-roles.ts */}
