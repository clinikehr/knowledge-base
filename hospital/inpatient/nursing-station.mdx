...
<Steps>
  ...
  <Step title="Send something to the patient without leaving the bedside">
    **Send to patient**, in the charting sheet's footer, opens the same three-step sheet the patient's record uses — forms, signature requests and consent documents in one list, with anything they already have open marked so it does not go twice. See [Send a form](/practice/forms/send-a-form).
  </Step>
