...
## The nine areas and their defaults
...
| **Maternity** | **Off** | The whole Maternity group — Today's Clinic, the Antenatal Register, Coverage, Labour Ward and Postnatal — and the Maternity report |
...
**Maternity** is the near-exception to the three-things-at-once rule above: it brings a navigation group, a report, and two blocks on this same **Facility** tab rather than a settings tab of its own.

An area that is on may bring its own settings with it. **In-patient care** adds **Nursing observations** and **Nursing shifts**; **Maternity** adds **Antenatal contact schedule** and **Partograph**; **Front desk** adds **Front desk waiting**; **Records** adds **Records chase**. Those sub-sections disappear with their area too.

**Surgical theatre and Maternity are the two that start off**, because most sites that sign up neither operate nor run an antenatal clinic. If you have a theatre, switch it on now — [step 4](/hospital/get-started/theatre-setup) needs it. Maternity can wait until you are ready — see [Set up antenatal care](/hospital/get-started/maternity-setup).
