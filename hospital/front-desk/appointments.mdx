...
<Availability editions={['clinic']} plans="All plans — a telehealth link on a booking needs any paid plan" roles="Any staff member the Front Desk group is shown to" note="Needs the Front desk care area" />
...
| **Search patients, services, out of office...** | Narrow to one person, one service or an Out of Office block |
| **More filters** | Opens **Calendar Filters** — **Appointment Status**, **Urgency Level**, **Out of Office**, **Event Types**, **Task Status**, **Task Priority**, **Venue Type**, **Services** and **Staff Members** |
...
To block time so clients cannot book it, use the **Out of office** tab — see [Add Out of Office](/hospital/front-desk/out-of-office).
...
<Steps>
  <Step title="Select Create">
    Or select the empty slot you want on the calendar — both open the same **Initial Entry** sheet, on the **Appointment** tab. The other tabs, **Event**, **Task** and **Out of office**, put something in the day that is not a patient booking.
  </Step>
...
| Create, move or cancel a booking | Anyone who can open the calendar | Nothing — the calendar has no role check of its own and no permission governs it |
...
<AccordionGroup>
  ...
  <Accordion title="What is the difference between an Appointment, an Event and a Task?">
    An **Appointment** is a patient booking. An **Event** is something in the day that is not a patient — a meeting or a seminar. A **Task** is a job somebody has to do, with a priority and a status, and no slot on anyone's clinic list. When a task falls due, the person it is assigned to — or, with nobody assigned, whoever created it or your clinic's email address — gets an email that says only that a task is due and when — never what the task is or who it is about. **Open task** in that email opens the task on the calendar after they sign in.
  </Accordion>
