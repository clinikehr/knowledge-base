...
<Availability editions={['clinic']} plans="Enterprise" roles="Any staff member can open it; only a pharmacist can dispense" note="Needs the In-house pharmacy care area · with Role Navigation on, no built-in role rule includes the Pharmacy Station" />
...
The station checks no role when you open it — anyone who can reach the **Pharmacy** group can read it. The group needs the **In-house pharmacy** care area in **Settings → Facility**, and an owner can hide it from a role under **Settings → Role Navigation** — where no built-in role rule reaches the Pharmacy Station, so switching it on takes this page away from your pharmacist until you add it to their rule.

Role, not permission, decides what you may do inside it. The dispensing permissions belong to the standalone Pharmacy edition and are inert in a hospital workspace.
...
| **Dispense** a script | Pharmacists only | The signed-in user's role — the dispense itself is refused for anyone whose role is not **pharmacist**, the owner included |
