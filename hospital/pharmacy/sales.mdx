...
<Availability editions={['clinic']} plans="Enterprise" roles="Owners, managers, accountants, pharmacists, doctors and nurses" note="Needs the In-house pharmacy care area · with Role Navigation on, no built-in role rule includes the Pharmacy Station" />
...
The heading reads **Sales Dashboard** — "Track and analyze your pharmacy sales performance". Four controls sit beside it: a date-range picker, which opens on the last thirty days; **Export**, which downloads the period as a spreadsheet; **Debtors**, which opens what is still owed; and **New Sale**, which takes you to the [till](/hospital/pharmacy/pos).
...
**Empty state.** With nothing matching you get **No sales found** and **Try adjusting your search or filter criteria** — check the date range and the status filter before concluding the till took nothing.

## Find a sale and reprint its receipt

<Steps>
  <Step title="Set the period">
    The date range filters everything on the page. A sale from last month will not appear while the range covers only this week — widen it and it comes back.
  </Step>
...
**Role** decides whether the screen opens: owners, managers, accountants, pharmacists, doctors and nurses pass, and everyone else is refused. **Sales** needs the **In-house pharmacy** care area, and an owner can hide the Pharmacy group from a role under **Settings → Role Navigation** — no built-in role rule reaches the Pharmacy Station, so switching it on takes this page away from your pharmacist until you add it to their rule.

Once you are in, nothing here is narrowed further. The refund and cost-visibility permissions belong to the standalone Pharmacy edition and are inert in a hospital workspace.

| Action | Who by default | Governed by |
| --- | --- | --- |
| Open Sales and search it | Owners, managers, accountants, pharmacists, doctors, nurses | Role — a fixed list on this page |
| Reprint or download a receipt | The same people | Role → route |
| **Edit sale** | Anyone who can open the page | Nothing |
| **Return / void all** | Anyone who can open the page | Nothing |
| See cost, margin and **Net position** | Anyone who can open the page | Nothing — the cost-and-margin permission is a Pharmacy-edition one and does not apply here |
...
A standalone pharmacy can withhold cost and margin from a cashier, and where it does the row or column is left out rather than shown as **0**. A hospital workspace has no such switch, so everyone who can open this page sees every figure on it.
...
<AccordionGroup>
  <Accordion title="No sales found, but the till took money today">
    Check the date range and the status filter first — a filter set to **Refunded**, or a range covering only today when you want last week, produces exactly this.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Can I export the sales list?">
    Yes — **Export** downloads the date range you have picked as a spreadsheet, with sales grouped by category, a subtotal for each and a reconciliation to what the till took. It is the same workbook the Executive dashboard's **Export accounts** produces, so the two cannot disagree about a period.
  </Accordion>
