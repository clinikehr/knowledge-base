...
Each medication also offers **Download PDF** — a server-generated prescription document (medication, dosage, frequency, duration, quantity, the prescriber's name and licence, and the dispenser's once it is dispensed). A prescription not yet dispensed is stamped **"Not yet dispensed"**, so a partly-filled script can never be mistaken for the final record.
