...
<Availability editions={['clinic']} plans="Enterprise" roles="Pharmacists, doctors, nurses, managers and owners" note="Needs the In-house pharmacy care area · with Role Navigation on, no built-in role rule includes the Pharmacy Station" />
...
**Role** decides whether the till opens: pharmacists, doctors, nurses, managers and owners pass, and everyone else is refused. **POS** needs the **In-house pharmacy** care area, and an owner can hide the Pharmacy group from a role under **Settings → Role Navigation** — where note that no built-in role rule reaches the Pharmacy Station, so switching Role Navigation on takes the till away from your pharmacist until you add it to their rule.

Nothing at the counter is governed by an individual permission: the till and cash-drawer permissions belong to the standalone Pharmacy edition and are inert in a hospital workspace.

| Action | Who by default | Governed by |
| --- | --- | --- |
| Open the till and complete a sale | Pharmacists, doctors, nurses, managers, owners | Role — a fixed list on this page |
...
| Return or void a sale | Anyone who can open [Sales](/hospital/pharmacy/sales) | Role — the Sales page's own list, not a permission |
| See cost and margin figures | Anyone who can open [Sales](/hospital/pharmacy/sales) | Nothing — the cost-and-margin permission is a Pharmacy-edition one and does not apply here |
