...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="Owner-only to change, except Patient ID prefix, which a manager can also change" />
...
One card, **General Information**, with a single **Save Changes** button at the bottom. Every field on it is owner-only, with one exception: a manager can also change **Patient ID prefix**, and sees a **Save patient ID prefix** button for it. An accountant sees every field filled in and disabled.
...
<AccordionGroup>
  <Accordion title="Everything is filled in but I cannot type in any of it">
    This tab is owner-only. Managers and accountants can read it; the fields render disabled and the **Save Changes** button is not shown. A manager can still change **Patient ID prefix** and save it with **Save patient ID prefix**.
  </Accordion>
