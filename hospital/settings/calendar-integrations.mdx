...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="Any staff member can connect their own account — from here, or from Settings → Calendars on their own account if they cannot open clinic Settings; the clinic-wide feeds and disclosure policy are owner or manager only" />
...
- **Blocks booking** — the provider's slots at that time disappear from availability, and a booking, move or repeat made from the calendar over that time is refused, so nobody double-books them. There is no appointment, no patient, no invoice and no clinical record. New feeds start here.
...
<AccordionGroup>
  ...
  <Accordion title="I can connect my own account but see no subscription list">
    Subscriptions are clinic-wide governance and load for owners and managers only.

    Connecting your own account is a personal action and is open to any staff member. If your role cannot open clinic Settings, do it from your own account instead: <Path steps={['Your avatar', 'Settings', 'Calendars']} />. Pick the clinic there — a calendar is connected per clinic, so someone who works in two has two separate connections.
  </Accordion>
...
{/* source of truth: components/clinic/dashboard/settings/calendar-integrations-settings.tsx, components/clinic/dashboard/settings/connected-calendars-section.tsx, choose-calendars-sheet.tsx, db/actions/external-calendars.ts */}
