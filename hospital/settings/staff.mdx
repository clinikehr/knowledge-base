...
The **Specialty** step in the invite wizard is really the person's role — it decides which pages they can open. **Clinical Specialty** is a separate thing entirely: what they practice (Family Medicine, Dermatology, and so on), which never changes what they can do.

Select **Set Clinical Specialty** from a colleague's row in **Staff Members** to choose one **Primary specialty** and any number of **Additional specialties**, from a standard clinical list or one your hospital has added. The primary specialty appears on that person's signature line on notes and reports, and can be used to filter the provider list when booking an appointment.
...
| **Packages & gift cards** | Sell packages · Sell gift cards · Redeem at checkout (package sessions, gift cards and promo codes on an invoice) |

<Note>
  **Prescribe medications** governs who may start an e-prescription and send it to a pharmacy. It works alongside the ePrescribe seat, and both are required: the seat is what your clinic pays for, this permission is who you have decided may use it.

  It is not a check of anyone's licence to prescribe. ClinikEHR cannot know which of your clinicians hold prescribing rights in your jurisdiction, so this is your decision to record, not ours to infer. Grant it deliberately.

  Staff who are not governed at all are unaffected, as with every permission here.
</Note>
