...
<Note>
  **A shift adds time on top of normal working hours — it never removes any.** Weekly working hours (set under Booking Settings) stay the default; a scheduled shift simply adds bookable time on its date. A service line that never uses shifts sees no change.
</Note>
