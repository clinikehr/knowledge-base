...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="Needs the Inventory care area · the reset is owner-only" />
...
<AccordionGroup>
  ...
  <Accordion title='"This inventory cannot be reset"'>
    Vaccine administrations, ward medication administrations, adverse-event reports or a clinical dispense against a patient reference these lots, and the reset is refused to protect that traceability. Correct quantities with a stock count instead.

    **A till sale to a registered customer is not one of these.** A sale records who bought an item, not what was administered to whom, so it does not block a reset — a pharmacy that registers its regulars keeps the clean re-import it needs while onboarding.
  </Accordion>
