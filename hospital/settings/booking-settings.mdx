...
A schedule is the unit of work here: a day, a time range, a slot length, a maximum number of bookings, a venue and optionally a named staff member. A hospital normally has several — one per clinic session, per provider or per venue. Select anywhere on a row — in any of the five tabs — to open it for editing. Times are shown and entered in 12-hour clock with AM/PM throughout.
...
**A named provider's schedule replaces the shared one for them on that day — the two never fight over the same time.** Giving a provider their own session on a day the hospital-wide schedule also covers is not a conflict; their own hours are simply what's offered for them, and the shared schedule keeps covering everyone else. A conflict warning only appears when two schedules would genuinely double-book the SAME provider (or two shared schedules) at an overlapping time. The database also refuses a genuine overlap or an exact duplicate outright, even from two people saving at once.
...
{/* source of truth: components/clinic/appointment/settings/appointment-settings.tsx, appointment-schedule-form.tsx, lib/settings-tabs.ts, lib/appointment/availability-conflict.ts */}
