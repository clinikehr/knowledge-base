...
<Availability editions={['clinic', 'lims']} plans="Varies by surface — see the table below" roles="Owner or manager — portal branding is owner-only" />
...
| Public surface | Branded at | Plan | Who |
| --- | --- | --- | --- |
| The patient portal | <Path steps={['Settings', 'Client Portal', 'Branding']} /> | Any paid plan | Owner |
| Your booking page | <Path steps={['Settings', 'Booking Page Designer', 'Design']} /> | All plans | Owner, manager or accountant |
| Your store | <Path steps={['Settings', 'Store Page']} /> | Starter and above | Owner or manager |
| Your membership page | <Path steps={['Settings', 'Membership Page']} /> | Essential and above | Owner or manager |
| Your contact form | <Path steps={['Settings', 'Contact Form']} /> | Starter and above | Owner or manager |
| Your forms | <Path steps={['Settings', 'Forms Settings']} /> for new forms; each form's own **Design** panel for that form | All plans | Owner |
| Lab reports (Diagnostics) | <Path steps={['Settings', 'Lab Workflow', 'Report Branding']} /> | All plans | Owner, manager or accountant |
...
<AccordionGroup>
  ...
  <Accordion title="The fields are read-only">
    Two different reasons, and the table above says which applies. Either you are a manager or accountant on a surface that is **owner-only** — the patient portal is the one that is — or your **plan** does not include that surface, in which case the whole card sits behind an upgrade panel rather than reading as merely locked. See [Roles](/platform/team/roles) and [Plans](/platform/plans/overview).
  </Accordion>
