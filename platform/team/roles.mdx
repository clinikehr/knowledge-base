...
<Availability editions={['all']} plans="All plans" roles="Owner" note="Role Navigation, below, needs an Enterprise Clinic workspace." />
...
**Owners bypass the permission system entirely** inside their own workspace: they can perform every act their edition and plan allow. Managers also bypass it, but they do not inherit the owner's administration screens — inviting a colleague, changing a specialty and editing permissions are all on the owner-only **Actions** menu of **Staff Members**.
...
<Availability editions={['clinic']} plans="All plans" roles="Owner, Manager" note="Enterprise Clinic workspaces only — the clinic type set at creation, not a subscription tier." />
...
<Note>
  **That row menu is owner-only.** A manager or accountant can open **Settings** → **Staff & Teams** and read the list, but the **Actions** column — Change Specialty, Permissions and Remove Staff — is rendered only for the workspace owner, and so is **Invite Staff**. Managers administer people through the owner, not around them.
</Note>
