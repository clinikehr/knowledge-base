...
<Availability editions={['all']} plans="All plans, within your seat cap" roles="Owner" note="Invite Staff renders only for the workspace owner — a manager or accountant sees the staff list without it." />
...
| **Pending Invitations** | Invitation sent, not yet accepted. Shows **Sent** and **Expires** | Wait, or **Resend** to send it again and reset the 72-hour clock. **Cancel** withdraws it |
| **Pending Seat Payments** | The seat checkout was not completed | **Retry Payment**, or cancel |

**Resend** is for the ordinary case where the email never arrived — it went to spam, or the 72 hours ran out before they got to it. It sends the same invitation again and gives it another 72 hours; it does not create a second invitation and does not charge a second seat. Only an owner or manager can resend.
