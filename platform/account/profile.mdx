...
<Warning>
  Keep the expiry date current. Once a licence passes its expiration date, ClinikEHR marks it **EXPIRED** on the signature block of every report you sign — lab reports, radiology reports and clinical notes — where a recipient or a regulator will see it.

  You are not blocked from working, and the licence is still printed rather than hidden: a report with no credentials at all is worse than one that states the position honestly. This page also warns you in the 60 days before a licence lapses, so there is time to renew.

  Licences with no expiration date recorded are printed as before, with no status either way.
</Warning>
