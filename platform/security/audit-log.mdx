...
| **Plan Limit** | Your plan stopped something — publishing past the allowance, an envelope past the month's allowance, a feature your plan does not include |
...
- **Changes to your Sharable Documents are recorded — once per save.** Saving the card writes one entry under **Appointment Default Documents** that says how many documents were added, removed or changed. A save that changes nothing writes nothing, and the entry never repeats a document's title.
- **Refusals are recorded.** When someone tries an action their permissions do not allow, that attempt is written as **Access Denied**. A pattern of them is worth a conversation. When your **plan** stops something instead, the entry is **Plan Limit** and says which limit — a sign it may be time to change plan, not a security concern.

Entries read as plain sentences — roughly "_Consultation was viewed by Dr A. Bello for patient P-1042_" — so you can scan a day without decoding anything. Select an entry to expand its detail.
...
- **All resources** — narrow to one kind of record or screen, grouped by area: **Form Responses**, **Appointment Default Documents**, **Invoice** and so on. Your choice stays in the page address, so a filtered view is a link you can send to someone else who can open the audit log.
