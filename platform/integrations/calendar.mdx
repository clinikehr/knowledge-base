...
<Note>
  This tab lives inside **Settings**, which only an owner, manager or accountant can open. So a provider who is none of those cannot link their own calendar here; an owner or manager should add a read-only **subscription** to that provider's `.ics` feed instead and assign it to them.
</Note>
