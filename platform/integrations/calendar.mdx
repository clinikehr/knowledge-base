...
<Availability editions={['clinic']} plans="All plans" roles="Owner, Manager" note="A Google or Outlook account can only be linked by the person who owns it — nobody can connect a colleague's." />
...
| Who sets it up | Each person connects **their own** account — but this tab is only reachable by an owner, manager or accountant | An owner or manager, for any calendar |
...
Under **Connected Accounts**, select **Google** or **Outlook** and sign in. Each person links their own account — you cannot connect a colleague's calendar for them, and trying produces "That calendar can only be connected by its own staff member."

<Note>
  This tab lives inside **Settings**, which only an owner, manager or accountant can open. A provider who is none of those has a second door to the exact same connection: **Profile and Settings → Calendars**, on their own personal account, reachable regardless of what a workspace's navigation shows them. See [Your calendar connections](/platform/account/calendars). Failing that, an owner or manager can add a read-only **subscription** to that provider's `.ics` feed instead and assign it to them.
</Note>
...
- **Whether it blocks at all is a per-calendar switch.** Each row toggles between **Blocks booking** and **Info only**. On **Blocks booking** (where a new calendar starts), events prevent that provider being booked — online, in the portal and from the appointment book. On **Info only** they are visible but do not protect anything. Get this wrong and you either double-book or block a day for no reason.
...
- That provider's slots for that period are no longer offered on your [booking page](/practice/work/booking-page) — check it signed out, as a patient would.
...
<Columns cols={2}>
  ...
  <Card title="Your calendar connections" icon="/images/icons/user.svg" href="/platform/account/calendars">
    The same connection, reachable from your personal account instead of Clinic Settings.
  </Card>
</Columns>
