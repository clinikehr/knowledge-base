...
A hospital pharmacy unit does two separate jobs: a clinician writes the prescription, and pharmacy staff fill it from stock. This page covers both, and the batch and witness controls that sit between them.

<Path steps={['Sidebar', 'Pharmacy', 'Pharmacy Station']} />

<TaskHeader before="The pharmacy care area switched on, and the medication in stock in your inventory" time="2 minutes" after="A dispensed prescription with stock drawn down against the patient" />
...
<Steps>
  <Step title="Start it">
    Either open the [consultation](/clinic/clinical/consultation) and use the **Orders** tab, or open **Pharmacy Station** and select **New Prescription**. Prescribing from the consultation keeps the script attached to the encounter that justified it.
  </Step>
  <Step title="Choose the patient and date">
    Fill **Select Patient** and **Prescription Date**.
  </Step>
  <Step title="Add each medication">
    Use **Search medications...** to add a line, then complete \*\*Mode \*\*\*, \*\*Quantity \*\*\*, \*\*Dosage \*\*\*, \*\*Frequency \*\*\*, \*\*Duration \*\*\* and, if it helps whoever dispenses, **Instructions (Optional)**. Repeat for every drug on the script.
  </Step>
...
- The stock item's on-hand figure has dropped by the quantity you dispensed — see [Inventory](/pharmacy/inventory/index).
...
<AccordionGroup>
  ...
  <Accordion title="The witness field will not accept my colleague">
    It only lists staff in this workspace who are permitted to witness. If the person is standing with you but not in the list, they need a staff account — see [Invite staff](/platform/team/invite-staff).
  </Accordion>
  ...
  <Accordion title="There is no Pharmacy group in my sidebar">
    Either the pharmacy [care area](/platform/team/care-areas) is off, or your workspace is not on the Enterprise plan. See [Change your plan](/platform/plans/change-plan).
  </Accordion>
</AccordionGroup>
