...
Signing up creates both your personal account and the clinic workspace it owns. It is a four-step wizard, and it finishes with an email you must open before you can log in.

<Path steps={['clinikehr.com', 'Sign up']} />

<TaskHeader before="A permanent personal or work email address you can open right now" time="3 minutes" after="A verified account and an empty clinic workspace, ready for setup" />
...
<Steps>
  ...
  <Step title="Describe your clinic">
    **About your clinic** asks for **I am a…** (your role), **The kind of care I provide is…**, **Clinic / practice name**, and **Practice size** — one of **Solo (1–3 providers)**, **Team (4–20 providers)** or **Enterprise (21\+ providers)**.

    Practice size is the one answer with a lasting effect: it decides whether your workspace opens with the solo/team layout, which calls the people you see **Clients**, or the hospital layout, which calls them **Patients** and groups the sidebar by service line. See [The three editions](/start/editions).
  </Step>
...
- The plan comparison page. Also normal, not an error — pick a plan or start the trial from there. See [Choose a plan](/clinic/get-started/choose-plan).

Either way the sidebar now shows **Get Started**, and that is where the [eight-step checklist](/clinic/get-started) lives.

## If something goes wrong

<AccordionGroup>
  <Accordion title="'You already have an account'">
    The wizard returns you to step 2 with "An account with this email already exists." and a **Log in instead →** link. Use it. If you cannot remember the password, use **Forgot password** on the sign-in page rather than creating a second account with a different address — a second account is a second workspace, and patient data cannot be moved between them.
  </Accordion>
  ...
  <Accordion title="I chose the wrong practice size">
    Practice size sets the shape of your workspace. It is not a setting you can flip yourself — [contact support](/platform/help/contact-support) and ask for the change rather than starting again.
  </Accordion>
</AccordionGroup>
