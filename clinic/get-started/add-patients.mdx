...
Step 2 of the [setup checklist](/clinic/get-started). One record completes it. The intake form is three steps, and only two fields plus a status are actually required — everything else can wait.
...
On an empty list you get a card reading **Add your first client** (or **Add your first patient**) — "Register one client manually with the full intake form." — alongside an **Import clients** card, and an **Explore with sample clients** tour that saves nothing to your clinic.
...
<Steps>
  <Step title="Choose the record type">
    **Client type** (or **Patient type**) offers **Adult**, **Minor** and **Couple**. A minor gets a **Contact** tab for the parent or guardian, with a **Relationship** picker and a **Responsible for billing** checkbox; a couple gets **Partner 1** and **Partner 2** tabs.
  </Step>
...
- A toast reads **Client created successfully!** (or **Patient created successfully!**) and the record appears in the list.
...
<AccordionGroup>
  ...
  <Accordion title="I have a spreadsheet of existing records">
    Use the **Import clients** card on the empty state rather than typing them in one by one. Details in [Register patients](/clinic/front-desk/register-patients).
  </Accordion>
