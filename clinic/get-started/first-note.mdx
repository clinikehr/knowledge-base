...
This is step 1 of the [setup checklist](/clinic/get-started), and the step most clinics do first because it is the fastest way to see what the product actually does. Any saved note completes it — including a draft.

<Path steps={['Sidebar', 'Notes', 'New Note']} />

<TaskHeader before="Nothing. A client or patient record is optional for a draft, and required before you sign" time="2 minutes" after="A saved note, either a draft you can come back to or a signed, locked record" />
...
<Steps>
  ...
  <Step title="Attach the client or patient">
    Use the **Client:** picker at the top of the form — it says **Patient** on the enterprise plan. It is optional for a draft, required before signing, and the AI Note Taker refuses to run without it.
  </Step>
...
While it works you see **Generating Notes ✨**, then **Notes Ready!** with the draft and a reminder that it is _generated in memory only_ until you select **Apply to Form**. Applying it inserts the text under an `--- AI Generated
...
<AccordionGroup>
  ...
  <Accordion title="The AI Note Taker will not start">
    It needs a client or patient selected on the note. If the button is greyed out rather than complaining, your plan does not include it — see [Change your plan](/platform/plans/change-plan).
  </Accordion>
