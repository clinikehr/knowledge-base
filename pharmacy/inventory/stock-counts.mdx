...
<Availability editions={['pharmacy']} plans="All plans" roles="Anyone whose role reaches Inventory; a controlled count also needs a witness" />
...
| Open a count and key quantities | Anyone whose role reaches **Inventory** | Role — the toggle exists but nothing reads it yet |
| Approve a count and post the variances | Anyone whose role reaches **Inventory** | Role — except a controlled count, below |
...
The countersignature is the one real gate on this page, and the one to plan around: it sits with the **Dispensing Supervisor** bundle and **not** with **Pharmacy Manager**, because countersigning a controlled count is a pharmacist's act rather than a manager's. **Both halves are checked** — the person approving and the colleague they name as witness. If nobody on shift holds it, a controlled count cannot be approved — check before you start counting, not after.

**There is no permission for adjusting a count.** **Adjust stock counts** was offered on the permissions sheet until August 2026 and nothing ever read it, so it was removed; anyone who can open **Inventory** can key or approve an ordinary count.
