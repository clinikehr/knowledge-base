...
<Availability editions={['pharmacy']} plans="All paid plans, uncapped; a pharmacy with no active subscription is capped at 5 purchase orders" roles="Staff with the manage suppliers and purchase orders permission" />
...
A pharmacy that has never ordered anything sees an empty list, and — only when it has no active paid subscription — a **Free plan** strip counting purchase orders against a cap of five. Every paid pharmacy plan, Starter upward, is uncapped.
...
| Receive a delivery against a PO line | Anyone whose role reaches **Inventory** | Role — receiving is deliberately outside the purchasing permission |
