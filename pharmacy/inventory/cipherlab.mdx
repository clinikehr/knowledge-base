...
<AccordionGroup>
  ...
  <Accordion title="EVERY barcode is unmatched — the whole upload">
    Your catalogue has no barcodes recorded, so there is nothing for the scans to match. This is normal for a pharmacy whose catalogue was imported from a POS report, which carries no barcode column. The sheet says so when it detects it — "Only \{n\} of your products have a barcode recorded" — and the fix is one import: upload your old system's item list, with its UPC column, through **Inventory → Import**. The barcodes attach to your existing products, and this count's scans are kept — re-open it after the import and they match.
  </Accordion>
