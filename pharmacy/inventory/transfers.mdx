...
<Availability editions={['pharmacy']} plans="All plans" roles="Staff who can open Transfers — not a cashier" note="Needs a second pharmacy branch under the same owner" />
...
**Role** decides whether **Transfers** opens at all. A **Cashier / Till Operator** reaches only **POS** and **Sales**, so this screen is not theirs at any tier. **Permission** is the vocabulary an owner grants in: **Transfer stock between branches** — "Send and receive stock moves across the business" — is the one that names this act.
...
| Dispatch with **Transfer to branch** | Pharmacy Manager, Inventory & Purchasing | **Transfer stock between branches** |
| **Receive** an incoming transfer | Pharmacy Manager, Inventory & Purchasing | **Transfer stock between branches** |
| Receive a supplier delivery instead | Pharmacy Manager, Inventory & Purchasing | **Receive deliveries into stock** |
| See what the transferred stock cost | Pharmacy Manager, Inventory & Purchasing | **View cost prices and margin** |

Two roles are deliberately outside it. A **Dispensing Supervisor** holds the till authorisations — refunds, price overrides, the controlled-drug countersignature — and none of the stock ones, so they cannot dispatch. A **Cashier / Till Operator** holds only **Dispense and sell at the till** and **Open the cash drawer**.
