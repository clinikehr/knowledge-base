...
<AccordionGroup>
  ...
  <Accordion title="My file has a quantity column, but no stock arrived">
    Correct — the catalog import never writes stock, and the **How we read your columns** panel lists the quantity column as not imported. Stock is received, not declared: after importing the catalog, bring quantities in through **Receive stock** or a Stock Count, so every unit lands as a ledger entry that can be audited later.
  </Accordion>
