...
The role bundles say a **Dispensing Supervisor** holds the till authorisations and none of the stock ones, and that **Whole branch** carries **Receive deliveries into stock** and **Adjust stock counts** where **Own till** carries neither. Read that as intent, not as enforcement: neither of those two toggles is checked anywhere in the product yet, so a Dispensing Supervisor who can open **Inventory** can in fact receive against an imported catalogue. **View cost prices and margin** and **Manage suppliers and purchase orders**, the two rows below, are checked on the server.
