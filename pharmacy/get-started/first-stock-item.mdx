...
<Availability editions={['pharmacy']} plans="All plans" roles="Anyone whose role reaches Inventory — not a cashier" />

Step 2 of the [setup checklist](/pharmacy/get-started) — "Stock your shelves so you can dispense and sell from live inventory." One item of any kind completes it, and it is the one step the till genuinely depends on.

<Path steps={['Sidebar', 'Inventory', 'Add item']} />

<TaskHeader before="Your currency set, so the price you enter is the price you charge" time="3 minutes for the first one, under a minute once you have the habit" after="A product on the shelf list, priced, sellable at the till, and counted" />
...
<Steps>
  ...
  <Step title="Receive opening stock">
    **Opening stock (optional)** with **Into location** puts quantity on the shelf as you create the item. Enter what is physically there now, with its **Expiry date** and, if you like, its **Lot / batch number (optional)** — an item tracked by lot / expiry will not save stock without the date.
