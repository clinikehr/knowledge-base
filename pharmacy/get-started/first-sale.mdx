...
<Availability editions={['pharmacy']} plans="All plans" roles="Anyone whose role reaches POS — a cashier included" />
...
<AccordionGroup>
  ...
  <Accordion title="I cannot open POS at all">
    **POS** is missing from your sidebar, which is a *role* answer rather than a permission one — a pharmacy technician, for instance, reaches the till, while a receptionist-style role may not. There is no "may sell" permission to be granted: whoever reaches **POS** can ring a sale up. Ask an owner to change the role. See [Staff permissions](/platform/team/permissions).
  </Accordion>
</AccordionGroup>
