...
<Availability editions={['pharmacy']} plans="All plans, up to the plan's seat cap" roles="Owner" note="A manager or accountant can open the Staff tab, but Invite Staff is the owner's" />
...
<Steps>
  ...
  <Step title="Tune the permissions">
    ...
    The bundle sets sensible defaults: a **Cashier** gets none of the till authorisations — they can sell, because reaching **POS** is what allows that, but they cannot discount, override a price or reverse a sale. Leave it there unless you have a reason not to.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="A cashier only sees two screens">
    A person whose **Specialty** is **Cashier** reaches **POS** and **Sales** and nothing else. That is expected, not a fault — and it is their *specialty* that does it, not the permission toggles. The sidebar is built from the role; the toggles govern individual acts inside it.
  </Accordion>

  <Accordion title="The countersignature is deliberately not the manager's">
    **Witness a controlled-drug dispense** belongs to the **Dispensing Supervisor**. A **Pharmacy Manager** gets everything else except that *and* **View per-cashier takings** — the second is a default for no pharmacy role or tier at all, so it has to be granted to a named person. A countersignature only means something when it comes from a second, qualified person.
  </Accordion>
</AccordionGroup>
