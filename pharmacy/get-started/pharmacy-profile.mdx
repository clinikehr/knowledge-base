...
<Availability editions={['pharmacy']} plans="All plans" roles="Owner" note="A manager or accountant can open the tab, but every field is read-only for them" />
...
<TaskHeader before="Owner access — a manager or accountant can read this tab but not save it" time="3 minutes" after="A complete profile — name, address, phone number and email — carried onto everything you print and send" />
...
<AccordionGroup>
  ...
  <Accordion title="Settings is not in my sidebar">
    **Settings** opens for an owner, a manager or an accountant; anyone else meets "You don't have permission to access the settings module." Opening it is not the same as saving from it — on **General Settings** every field is read-only unless you are the owner. See [Staff permissions](/platform/team/permissions).
  </Accordion>
