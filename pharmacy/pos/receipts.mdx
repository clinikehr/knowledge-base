...
Receipts appear in two places, and neither is a screen of its own.

At the till, the **Sale Complete!** panel replaces the cart and offers **Print Receipt** and **New Sale**. On **Sales › Recent Sales**, every row's **⋯** menu carries **Print Receipt** and **Download PDF** under an **Actions** heading; both open the same dialog, **Receipt · \{sale number\}** — "Reprint on an 80mm/thermal printer or download a PDF copy." — with **Print** and **Download PDF** on it.

In the desktop app the till also carries **Printer settings**, which is where the paper form is chosen. In a browser that button is not there, because the browser's own print dialog handles the paper.

**A brand-new pharmacy** has nothing to reprint: **Recent Sales** is empty and the **⋯** menus with it. The first receipt you can reprint is the first sale you ring up.
