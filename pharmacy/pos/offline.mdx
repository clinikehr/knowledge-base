...
<Availability editions={['pharmacy']} plans="All plans" roles="Anyone whose role reaches POS — selling offline is the same act as selling" />
...
| --- | --- | --- |
| Ring up and complete a sale offline | Anyone whose role reaches **POS**, a Cashier included | Role — selling is not permission-gated |
| Print the provisional receipt | The same people | Nothing — printing is not gated |
| Open **Sync status** and select **Sync now** | Anyone at that terminal | Nothing — it is a device status view |
| Open the cash drawer during the outage | Anyone whose role reaches **POS** | Role — there is no permission for this; the toggle that implied one was removed in August 2026 |
