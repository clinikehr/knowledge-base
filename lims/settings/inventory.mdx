...
| **Where the till sells from** | Pins the POS to one selling location. The till then shows and draws that location's stock only; a shortfall there is recorded instead of quietly pulling from another room. | **Automatic** — the location holding the soonest-expiring stock |
...
<AccordionGroup>
  ...
  <Accordion title="The reset is blocked">
    ...
    A sale to a registered customer is not one of these, and does not block the reset.
  </Accordion>
