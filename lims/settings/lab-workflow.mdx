...
<Availability editions={['lims']} roles="Owner, manager or accountant" note="Only an owner or manager can save a mode change — an accountant sees the tab but is refused" />
...
<AccordionGroup>
  ...
  <Accordion title="I cannot save the mode">
    Opening Settings needs an owner, manager or accountant, but **saving** this tab is narrower still: the server accepts a mode change only from an **owner or a manager**, so an accountant can read the tab and select an option and will be refused on save. A bench scientist cannot open the screen at all. See [Roles](/platform/team/roles).
  </Accordion>
