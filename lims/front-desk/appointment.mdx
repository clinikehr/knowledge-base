...
**The second row** is the working row: **Search patients, services, out of office...**, **More filters** (with a badge counting how many are active), **Refresh**, **Current Schedule** (**Schedule** on a phone) and **Create**.
...
To block time so clients cannot book it, use the **Out of office** tab — see [Add Out of Office](/lims/front-desk/out-of-office).
...
<Steps>
  ...
  <Step title="Stay on the Appointment tab">
    The sheet has four tabs: **Appointment** · **Event** · **Task** · **Out of office**. **Event** is for anything that occupies the diary without a patient — a maintenance visit, a training block. **Task** is a to-do with a due date, not a slot.
  </Step>
...
<Steps>
  <Step title="Search by name or service">
    Enter a name, a service or an Out of Office note into **Search patients, services, out of office...**. This narrows the calendar itself rather than opening a separate result list.
  </Step>
  <Step title="Filter">
    Select **More filters** to open **Calendar Filters** — "Refine your view by status, service, or staff." The sections are **Appointment Status**, **Urgency Level**, **Out of Office**, **Event Types**, **Task Status**, **Task Priority**, **Venue Type**, **Services** and **Staff Members**. **Reset all filters** at the foot clears everything at once.
  </Step>
...
Role is the lever on this screen. The grantable lab permissions cover results (**Enter results at the bench**, **Verify and authorise results**, **Amend a verified result**), turnaround analytics and the referrer portal — and none of them governs the appointment book. Whoever can open the screen can use it.
...
<AccordionGroup>
  ...
  <Accordion title="What is the difference between an Event and a Task?">
    An **Event** occupies time on the calendar but has no patient — analyser servicing, a delivery window, a training block. A **Task** is a piece of work with a due date and a priority; it does not hold a slot. When a task falls due, the person it is assigned to — or, with nobody assigned, whoever created it or your clinic's email address — gets an email that says only that a task is due and when — never what the task is or who it is about. **Open task** in that email opens the task on the calendar after they sign in.
  </Accordion>
