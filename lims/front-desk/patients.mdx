...
**The register's toolbar** has **Search patients...**, a **Patient status** filter (**Active**, **Inactive**, **Prospective**, **Archived**), a **Billing type** filter (**Self pay**, **Insurance**), a **Waitlist** toggle that shows only self-registered requests awaiting a decision, and **Columns**, which lists every column under **Visible Columns**. Each filter shows a count while it is on and offers **Clear filter**; **Clear all** appears once any is set. **Patient ID**, **Full Name**, **Category**, **Age**, **Gender**, **Phone**, **Nationality**, **Consultations** and **Last Visit** are shown by default. **Lab Tests**, **Blood Type**, **Genotype**, **Email**, **Occupation**, **Country**, **Triages** and **Prescriptions** are hidden until you switch them on — turn on **Lab Tests** if you want to see at a glance who your repeat testers are.

**Each row's menu** offers **View details**, **Edit**, **Send to patient**, and on a waitlist row, **Approve request** and **Decline request**. **Send to patient** opens the same three-step sheet the patient's own record uses, so you can send a form, a signature request or a consent document without leaving the register.
...
Role is the lever here too. The grantable lab permissions cover results, turnaround analytics and the referrer portal — **Enter results at the bench**, **Verify and authorise results**, **Amend a verified result**, **View turnaround-time analytics** and **Access the referrer portal** — and none of them governs the patient register. What differs between people is which actions their role allows.
...
| **Send to patient** | Anyone who can open the register | Role → route |
