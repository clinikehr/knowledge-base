...
<Availability editions={['lims']} plans="All plans" roles="Writing and submitting a report is Radiologist, Doctor or Owner. A Radiology Technician and a Manager reach the screen but see it read-only." />
...
| --- | --- | --- |
| Open a report read-only | Owner, Manager, **Radiologist**, **Radiology Technician**, Doctor | Role |
| Write findings and save a draft | **Radiologist**, **Doctor**, Owner | Role — no permission grants this |
| Submit or update a report | The same people | Role |
| Put the report in the referring doctor's portal | The same people | Role — a study appears once its status is **Reported**; there is no release act |
...
| Download or archive the study | Anyone who can open the report | Role |
...
**Release a report to a referrer is not enforced.** The slug exists and the permissions sheet offers it, but no screen performs a per-report release: the referrer portal shows an imaging study the moment its status reaches **Reported**. Submitting the report *is* the release. A **Lab Scientist** does not reach the imaging screens at all.
