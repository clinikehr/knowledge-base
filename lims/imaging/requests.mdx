...
<Availability editions={['lims']} plans="All plans" roles="Raising and reporting studies is limited to imaging staff. A receptionist can raise a request; everyone else gets a view-only queue." />
...
| --- | --- | --- |
| Open and read the queue | Owner, Manager, **Radiologist**, **Radiology Technician**, **Receptionist** | Role → route |
| Raise a request | Owner, Manager, imaging staff, **Receptionist** | Role → route |
| Attribute an order to a referrer | Anyone who may raise a request | Role → route — the picker is on the order form |
| Report a study | Radiologists with reporting access | Role → route |
| Release a report to the referring doctor | **Referrer Liaison**, **Lab Manager**, Owner, Manager | Permission — **Release a report to a referrer** |
| See a referrer's account and their statements | **Referrer Liaison**, **Lab Manager** | Permission — **Access the referrer portal** |
| See the queue but change nothing | Everyone else with the route | Role → route, in view-only mode |

Staff without reporting access get the queue in view-only mode — they can see studies and track progress but cannot raise or report them. Attempting to submit returns **You are not authorized to submit radiology orders.** That is role enforcement, not a fault: a **Lab Scientist** does not reach the imaging screens at all, and a **Radiologist** reaches only them plus patient records. Access tiers — **Own bench**, **Whole lab**, **All sites** — widen how much of the lab a person sees; only **All sites** carries the referrer grants.
