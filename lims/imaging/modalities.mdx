...
<Availability editions={['lims']} plans="Professional Lab and above" roles="Anyone granted “Configure analysers and interfaces”. Radiologists and lab scientists do not hold it by default." />
...
<TaskHeader before="Professional Lab plan, the “Configure analysers and interfaces” grant, and whoever looks after your imaging machines available" time="10 minutes per machine" after="A registered modality whose finished studies attach to orders by themselves" />
...
Role decides whether **Modalities** appears at all. Permission decides whether you may change anything on it.

| Action | Who by default | Governed by |
| --- | --- | --- |
| Open **Modalities** | **Lab Manager**, anyone on the **All sites** tier, Owner, Manager | Permission — **Configure analysers and interfaces** |
| Register, pause or re-key a machine | The same people | Permission — **Configure analysers and interfaces** |
| Attach or discard a waiting study | The same people | Permission — **Configure analysers and interfaces** |
| Save the PACS, archive and viewer settings | The same people | Permission — **Configure analysers and interfaces** |
| Report the study once its images land | **Radiologist**, **Radiology Technician** | Role → route |

Configuring this page is a separate grant from anything else in imaging, because it changes how machines talk to the system rather than what a report says. A **Radiologist** reaches only the imaging screens and does not hold it; a **Lab Scientist** does not reach imaging at all; a **QC Officer** is review-only by design.
