...
This is the lab's own trading dashboard: how much work came in over the last month or six, what mix it was, and which tests and referrers actually earned the money. The page says so itself — "Trends and mix across drug testing, lab and imaging. Revenue = collected on orders created in" the period you picked.
...
A date-range picker sits beside the heading, with an **Export** button next to it. **Pick any window** — click a single day to see just that day, or a start day and an end day for a custom range of any length. It opens on the last thirty days. Everything below follows it, and the line under the heading names the period you picked.
...
<Steps>
  <Step title="Pick the range deliberately">
    A month shows you this screen's shape; six months shows you a trend. Because the window is yours to set, you can ask for a specific month — 1 to 31 August — rather than a rolling count of days back from today. Comparing two months still means reading the daily chart rather than switching windows: the tiles always total the whole period.
  </Step>
...
- The picker shows the period you chose, and the daily charts start and end on the dates you expect.
...
**Export** offers each part of the page as a file, in two formats, for exactly the window on screen.

| Slice | What is in it |
| --- | --- |
| **Day by day** | A row per day: drug testing, lab and imaging counts, and revenue |
| **By test** | A row per test: orders and revenue |
| **By referrer** | A row per referrer: orders and revenue |
| **Payment mix** | A row per payment status: orders, collected and billed |
| **Summary** | Orders, revenue and the patient counts |

Only the slices that have something in them are offered — a lab with no attributed referrals is not given an empty referrer file.

**CSV** opens in Excel, Numbers or Sheets. **Excel (.xlsx)** is a workbook that opens on an **Overview** sheet, and every sheet carries the lab's name, the period and the currency at the top — so a tab forwarded to an accountant still says what it is. Both carry the period in the filename.

The two formats are built from the same figures, so a CSV and a workbook of the same window always agree.
