...
| Review the QC behind the run | Anyone who reaches Lab Station | Plan only — there is no QC permission, see [Quality control](/lims/quality/quality-control) |
| Accession or reject the specimen before it reaches you | Owner, Manager, Doctor | Role — see [Accession an incoming specimen](/lims/lab/accessioning) |
...
<AccordionGroup>
  ...
  <Accordion title="This order has no laboratory record, so nothing was saved">
    The specimen's laboratory record is missing, so there is nowhere to store the readings you just entered. Nothing was saved and nothing was finalized — re-run the lab accession step for this specimen, then enter the values again.

    If it happens a second time on the same order, stop and raise it with your lab director before recording anything further against that specimen. An order that reaches the bench without a laboratory record has skipped a custody step, and that is a question about the specimen, not about the form.
  </Accordion>

  <Accordion title="This urine specimen has no specimen-validity result recorded">
