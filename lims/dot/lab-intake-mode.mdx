...
<Availability editions={['lims']} plans="All plans — lab intake is included from Starter Lab" roles="Owner or Manager to switch the mode; any bench staff member to raise an intake order" />
...
Role decides whether **Lab Intake** appears. Permission decides what happens to the specimen after it is booked in — raising the intake order itself is not behind a separate grant, because whoever takes delivery is who must record it.

| Action | Who by default | Governed by |
| --- | --- | --- |
| Switch the workflow mode | Owner or Manager only | Role |
| Open **Lab Intake** and raise a **New Intake** | Owner, Manager, **Lab Manager**, lab scientists, phlebotomists | Role → route, plus the workflow mode |
| Accession the specimen and check its seals | **Lab Manager**, **Accessioning Clerk**, **Whole lab**, **All sites** | Permission — **Accession an incoming specimen** |
| Reject it on seal, volume or labelling | The same people | Permission — **Reject an unsuitable specimen** |
...
A **Lab Manager** does **not** hold **Verify and authorise results** by default — that comes from the access tier, not the administrative role, which is why a manager can book a specimen in and still find **Finalize Lab Report** greyed out. A **QC Officer** is review-only by design. A **phlebotomist** reaches test requests, this queue and patients, and nothing else.
