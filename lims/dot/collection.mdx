...
Role decides whether the collection screens open at all. Permission decides what happens to the specimen afterwards — collection itself is deliberately not behind a separate grant, because the person with the donor in front of them is the person who must complete the form.

| Action | Who by default | Governed by |
| --- | --- | --- |
| Open **DOT Testing**, raise an order, complete the CCF and the collection form | Owner, Manager, **Lab Manager**, lab scientists, phlebotomists | Role → route, plus the workflow mode |
| Accession the specimen when it lands | **Lab Manager**, **Accessioning Clerk**, **Whole lab**, **All sites** | Permission — **Accession an incoming specimen** |
...
A **phlebotomist** reaches test requests, the drug-testing queue and patients, and nothing else — which is exactly the collector's job. A **Lab Manager** does **not** hold **Verify and authorise results** by default; that comes from the access tier, not the administrative role. A **QC Officer** is review-only by design.
