...
<Availability editions={['lims']} plans="All plans" roles="Owner only — a manager or accountant can read the tab but every field is disabled and there is no Save button" />
...
<AccordionGroup>
  ...
  <Accordion title="Every field is greyed out and I cannot find Save">
    You are not the owner. Managers and accountants reach **General Settings** and can read it, but the whole form is disabled for them and the **Save** button is not rendered at all — this tab is owner-only to write. Ask the account owner to make the change.
  </Accordion>
