...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="Inviting, editing and removing staff is owner-only" />
...
| --- | --- | --- |
| **Staff Members** | Everyone currently in the practice: **Name** (the practice owner carries an **Owner** badge), **Email**, **Specialty**, **Clinical Specialty**, and **Last active**. | Everyone who can open Settings |
...
| Row **Actions** menu | **Change Specialty** · **Set Clinical Specialty** · **Permissions** (where your practice has permission-governed modules) · **Require MFA on sign-in** · **Remove MFA requirement** · **Reset two-factor** · **Grant emergency access** · **Remove Staff**. The owner's own row has no menu. | Owner only |
...
<Warning>
  **Seats are billed on this edition.** **Essential** includes 3 seats and **Team** includes 5, in the base price; every billable seat beyond that is charged monthly, at the rate shown next to the seat counter. **Essential** caps at 5 seats total; **Team** and **Enterprise** are uncapped but still charge for every seat past what is included. Three roles are free and count towards nothing: **supervisor**, **biller** and **scheduler**. The wizard's final step shows the exact monthly change before you send.
</Warning>
...
**Clinical Specialty is different from Specialty.** **Specialty** (the column and the **Change Specialty** action) is what a colleague signed in AS — the role that decides their permissions and which pages they can open. **Clinical Specialty** is what they PRACTICE — Family Medicine, Dermatology, and so on — and never changes what they can do in ClinikEHR.

Select **Set Clinical Specialty** from a colleague's **Actions** menu to choose one **Primary specialty** and any number of **Additional specialties**, searched from a standard clinical list or a specialty your own practice has added. The primary specialty:

- appears on the **Clinical Specialty** column and on that clinician's signature line on notes and reports
- can fill in the specialty code an insurance claim needs, when nothing more specific was entered on the claim itself
- can be used to filter the provider list when booking an appointment
