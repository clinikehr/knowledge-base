...
Book a test appointment for a service you scoped a document to. The document should appear on that appointment without you attaching it, and the client should receive it. Open [Documents](/clinic/clinical/documents) to confirm it is listed against them.

## Common issues

<AccordionGroup>
  <Accordion title="There is nothing to select">
    Defaults are chosen from templates that already exist. Create or activate a document template first — see [Documents](/clinic/clinical/documents) — then come back and select **Refresh**.
  </Accordion>
