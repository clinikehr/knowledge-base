...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="The Danger Zone is owner-only" />
...
<TaskHeader
  before="Nothing — the tab is there from day one"
  time="10 minutes"
...
<AccordionGroup>
  <Accordion title="There is no Inventory tab in my settings">
    You are signed in as neither owner, manager nor accountant — those three are the roles that reach Settings at all. The tab itself is present on every solo and team practice: the care-area switch that can remove it is an Enterprise control, and the area ships on. See [Care areas](/platform/team/care-areas).
  </Accordion>
