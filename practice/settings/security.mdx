...
<Availability editions={['clinic']} plans="All plans" roles="Owner to change · manager and accountant read-only" />
...
Two things people expect to find here are elsewhere:
...
<Availability editions={['clinic']} plans="Essential and up" roles="Owner or manager" />

A second card, **Access alerts**, below Security Settings. It sets how sensitive the **Unusual access alerts** on restricted clients are (see [Unusual access alerts](/practice/work/client-security#unusual-access-alerts)) — every secured client record still gets checked, this only changes when a check turns into a notification to the owner.

| Control | What it does | Default |
| --- | --- | --- |
| **Records opened / Within (minutes)** | How many distinct secured records one team member has to open, and inside how many minutes, before the "many secured records opened" alert fires. 1–100 records, 1–1440 minutes | 5 records within 60 minutes |
| **Times granted / Within (days)** | How many times emergency access has to be granted to one team member, and inside how many days, before the "repeated emergency access" alert fires. 1–100 times, 1–365 days | 3 times within 30 days |
| **Requests / Within (days)** | How many times one client may ask, from their own portal, to restrict their own record before the portal asks them to wait. 1–50 requests, 1–90 days | 5 requests within 7 days |
| **Default hidden categories for a guardian/proxy portal view** | Which sensitive document categories (behavioural health, substance use, reproductive health, HIV status, other) start hidden from a guardian or family member who reaches a client's portal record through a shared contact email. Applies to any client who has not had this set individually on their own Security tab, under **Proxy access limits** — that always wins over this clinic-wide default | Every category hidden |

On a plan below Essential, the card shows an **Upgrade** button instead and nothing can be changed — the alerts themselves still run on the built-in defaults above. Below Essential, or for anyone who is not the owner or a manager, the fields are read-only or the card does not offer changes at all.
...
{/* source of truth: components/clinic/dashboard/settings/security-settings.tsx, access-alert-settings.tsx, security-alerts-card.tsx (rendered on Staff & Teams) */}
