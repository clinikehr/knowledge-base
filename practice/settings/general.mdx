...
<Availability editions={['clinic']} plans="All plans" roles="Owner, manager or accountant" note="Only an owner can save changes here" />
...
| **Patient ID prefix** | The letters before the number on every new client's ID — **CMC**-0134. Two to six letters or numbers. Leave it empty to use the first three characters of your practice name. Existing clients keep their IDs, and numbering carries on from where it is. | Empty (derived from your name) |
| **Age of majority** | The age at which a new client is registered as an adult rather than a minor. Choose any age from 16 to 21. | **18** |
| **Timezone** | A searchable list of every timezone. "Used for scheduling and appointment reminders." | **UTC** |
| **Country** | Where your practice operates, from a searchable list. It decides which electronic signature statement people agree to before signing a document you send. While it is empty, your billing currency decides instead. | Not set |
...
Every field is editable by the **owner** only, except **Patient ID prefix** and **Age of majority**, which a **manager** can also change. An accountant sees the same tab with the values filled in, the fields greyed out, and no save button.
...
Below General Information, a separate **Business hours** card sets what your practice considers a normal working day, one switch and a start/end time per weekday, shown in the timezone set above. A day left switched off counts as closed.

This is what the **Unusual access alerts** out-of-hours check on the **Security** tab (see [Restrict a client to assigned staff](/practice/work/client-security#unusual-access-alerts)) compares an access against. A day with no hours set at all counts as closed too, so a practice that has never opened this card gets flagged more often, not less — not fewer alerts, until you set your real hours here.
...
| **Age of majority** | Changes the suggested client type on the registration form for anyone registered from now on, and the warning shown when a stored type disagrees with the date of birth. It also changes how the analytics tab splits clients into adults and minors. Nobody already registered is reclassified. |
...
<AccordionGroup>
  <Accordion title="The fields are greyed out and there is no Save Changes button">
    You are a manager or an accountant. This tab is owner-only to edit — you can read it but not change it. A manager can still change **Patient ID prefix** and **Age of majority**. Ask the practice owner, or see [Roles](/platform/team/roles).
  </Accordion>
...
{/* source of truth: components/clinic/dashboard/settings/general-settings.tsx, components/clinic/dashboard/settings/business-hours-settings.tsx, lib/onboarding-steps.ts */}
