...
Every field is editable by the **owner** only, except **Patient ID prefix**, which a **manager** can also change and save with **Save patient ID prefix**. An accountant sees the same tab with the values filled in, the fields greyed out, and no save button.
...
<AccordionGroup>
  <Accordion title="The fields are greyed out and there is no Save Changes button">
    You are a manager or an accountant. This tab is owner-only to edit — you can read it but not change it. A manager can still change **Patient ID prefix**. Ask the practice owner, or see [Roles](/platform/team/roles).
  </Accordion>
