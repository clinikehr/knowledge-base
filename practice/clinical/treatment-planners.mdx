...
<Steps>
  ...
  <Step title="Set a review date (optional)">
...
At the top of an open plan, **Review by** shows the date the plan is due for review. On an active plan, pick a date and select **Save review date**, or select **Clear** to remove it. The date is saved on its own, separately from **Save progress**.

In the plans list, an active plan whose review date has passed is marked **Overdue**, and one due within the next seven days is marked **Due soon**. Completed and discontinued plans are never flagged.
