...
<Availability editions={['clinic']} plans="All plans — signing and locking, AI Rewrite and AI Autocomplete all need Starter or above" roles="Any clinical staff member" />
...
Both need Starter or above — they are switched off on Free, not held back for a higher tier. To draft a whole note from a recording instead, use [the AI Note Taker](/practice/clinical/ai-notes) — a separate add-on, not a plan feature.
...
<AccordionGroup>
  ...
  <Accordion title="AI Rewrite or AI Autocomplete shows a flag and will not run">
    Your practice is on Free — both need Starter or above, and the tooltip reads "Upgrade to use AI Rewrite". They are separate from the AI Note Taker, which is an add-on bought per person.
  </Accordion>
