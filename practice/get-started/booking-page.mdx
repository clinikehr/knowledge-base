...
<Steps>
  ...
  <Step title="Set the booking and payment rules">
    The **Settings** tab holds **Allow Online Booking**, **Clinic Timezone**, **Advance Booking Days** ("How many days in advance patients can book") and the **Booking Confirmation Message**. **Payment Settings** below it controls **Accept Payments**, **Require Upfront Payment** and the **Refund Policy**, and **Booking policy** holds the cancellation and deposit wording clients agree to. Deposits themselves are set on each service. **Currency** is shown there but is not editable: it is your workspace's billing currency, set once under **Billing & Plans**, and the public page has always used that rather than anything set here.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Some days are striped and cannot be picked">
    Those days are inside an Out of Office block on your calendar — for the whole clinic, or for the provider the visitor chose. Visitors see "Striped days are unavailable." under the calendar, and a day that is only partly blocked shows the times that are left. Nothing about the block is shown beyond that — not its note, and not who added it. Remove or shorten the block from your calendar to open those times again.
  </Accordion>
...
Handling the bookings that arrive is covered in [Booking page](/practice/work/booking-page).
