...
<Availability editions={['clinic']} plans="All plans" roles="Any staff member with access to the calendar" note="Group sessions need Essential; recurring bookings need Team." />
...
<Steps>
  ...
  <Step title="Add notes and check the reminders">
    **Internal Memo** (**ADD**) opens **Symptoms Description**, **Patient Notes** and **Clinic Notes** — all staff-visible only.
    ...
    Under **Notification**, **Email** / **Auto-sent** is on by default, so the person is emailed a confirmation when you book. Reminders and confirmations go by email only — there is no SMS or voice channel today.
  </Step>
  <Step title="Book it">
