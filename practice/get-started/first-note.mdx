...
<Availability editions={['clinic']} plans="All plans" roles="Any clinical staff member" note="AI Rewrite and AI Autocomplete need Essential or above; the AI Note Taker is a separate per-seat add-on." />
...
<Steps>
  ...
  <Step title="Fill the sections">
    Type into each section. `/` opens the snippet menu anywhere in a note body, so a phrase you use constantly can be inserted rather than retyped — see [Note templates and snippets](/practice/clinical/note-templates).
  </Step>
  <Step title="Save it">
...
<Note>
  AI output is a draft for you to check, never a record on its own. Read every section, correct what is wrong, and only then save or sign. More in [AI clinical notes](/practice/clinical/ai-notes).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="A template has the wrong sections for my specialty">
    Build a note form with the sections you need — select **Create a note form** on **Templates** — or start from one in the template gallery. See [Note templates and snippets](/practice/clinical/note-templates).
  </Accordion>
