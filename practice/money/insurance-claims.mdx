...
<AccordionGroup>
  ...
  <Accordion title="The eligibility check says Ineligible">
    Check the member number, the payer and the date of birth on the client's coverage; one wrong digit reads as no cover. If the details are right, the cover has genuinely lapsed — raise an ordinary [invoice](/clinic/revenue/invoices) instead.
  </Accordion>
  ...
  <Accordion title="Nothing appears under Unbilled appointments">
    Only completed appointments with no claim against them are listed. If yours are still open, close them off first — see [Appointments](/practice/daily/appointments).
  </Accordion>
</AccordionGroup>
